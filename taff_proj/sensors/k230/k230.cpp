...
void tx_send(uint8_t type, const uint8_t* payload, uint8_t len) {
    uint8_t buf[common::frame::kMaxFrame];
    size_t n = common::frame::encode(type, payload, len, buf);
    if (n == 0) return;
    drivers::uart::write(drivers::uart::Id::K230, buf, n);
}
...
void on_reset() { g_vision = {}; g_link = {}; g_seq = 0; tx_send(TYPE_RESET_ACK, nullptr, 0); }
...
void init() {
    g_rx.flush(); g_parser.reset(); g_vision = {}; g_seq = 0; g_link = {};
    g_link.last_rx_ms = drivers::systick::now_ms();
